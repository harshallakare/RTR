...
int main(void)
{
	...
	printf("********** DATA 1 **********\n\n");
	printf("Enter Integer Value For 'i' Of 'struct MyData datal' : ");
	scanf("%d", &data1.i);

	printf("\n\n");
	printf("Enter Floating-Point Value For 'f' Of 'struct MyData datal' : ");
	scanf("%f", &data1.f);

	printf("\n\n");
	printf("Enter 'Double' Value For 'd' Of 'struct MyData datal' : ");
	scanf("%lf", &data1.d);

	printf("\n\n");
	printf("Enter Character Value For 'c' Of 'struct MyData datal' : ");
	data1.c = getch();
	printf("%c", data1.c);
...
struct MyData AddStructMembers(struct MyData md_one, struct MyData md_two, struct MyData md_three)
{
	...
	answer.i = md_one.f + md_two.f + md_three.f;
	answer.i = md_one.d + md_two.d + md_three.d;
	return(answer);
}